...
void Statistics::print(const fs::path &downloadsPath) const
{
    // Log Table Width
    int LABEL_WIDTH = 32;
    ...
    std::vector<std::string> cs = {"Images", "PDFs", "Videos", "Audio", "Code", "Zips", "Executables and Installers", "Documents", "Others"};
    ...
    // "%I:%M%p | %d %B, %Y"
    logFile << "_downTidy: " << std::put_time(std::localtime(&now_time), "> %I:%M%p | %d %B, %Y") << "\n";

    logFile << "===================================="
            << "\n";

    logFile << "[log]_downTidy__Cleanup" << "\n";
